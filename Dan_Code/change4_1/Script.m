...
X_DH   = 0.0866;
X_C1   = 32365 * 1e1;
X_C3   = 0.79081* 40 * 1e5 * 1e1;
X_C4   = 0.00010761 * 7e2 * 1e2;
X_F1F0 = 1000 ;
...
[t,x] = ode15s(@Cell_dXdT,[0 60],x0,[],pars,conc,fixedpars,flags);

% Find steady-state fluxes and polynomials 
f = zeros(14,length(t)); 
for i = 1:length(t)
    [~,f1] = Cell_dXdT([],x(:,end),pars,conc,fixedpars,2); 
    f(:,i) = f1; 
end 
...
figure(6)
clf
plot(t,sumATP_c,'b',t,sumADP_c,'r',t,sumPi_c,'c')
title('Cytosol')
legend('ATP','ADP','Pi')
set(gca,'FontSize',20)
...
for i = 1:length(X_AtC)
    p(end) = X_AtC(i); 
    [t,x] = ode15s(@Cell_dXdT,[0 60],x0,[],p,conc,fixedpars,1);
...
figure(10); plot(X_AtC*1e3,Pi_c*1e3);
xlabel('ATP consumption rate')
ylabel('[Pi]_c (mM)')
set(gca,'FontSize',20)
