function [f] = Cell_dXdT(t,x,x_AtC)
...
ADP_i1 = ADP_c/P_i(iADP); % ADP^3-
ATP_i1 = ATP_c/P_i(iATP); % ATP^4-
...
H2PIi1 = PI_c * (H_c / Kh(iPI)) / P_c(iPI);
...
f(idPsi)   = ( 4*J_C1 + 2*J_C3 + 4*J_C4 - n_A*J_F1 - J_ANT - J_Hle ) / 3e-6;
...
f(iATP_i) = 0; %(J_ATP + J_ANT + J_AKi)/W_i; 
f(iADP_i) = 0; %(J_ADP - J_ANT - 2*J_AKi)/W_i; 
f(iAMP_i) = 0; %(J_AMP + J_AKi)/W_i;   
f(iPI_i)  = 0; %(-J_PI1 + J_PI2)/W_i; 

%  (iii) Buffer species
%f(iATP_c) = (-Rm_cyto*J_ATP - J_AtC + J_CKe + J_AKe)/W_c;  
f(iATP_c) = (Rm_cyto*J_ANT - J_AtC + J_CKe)/W_c;  

%f(iADP_c) = (-Rm_cyto*J_ADP + J_AtC - J_CKe - 2*J_AKe)/W_c; 
f(iADP_c) = (-Rm_cyto*J_ANT + J_AtC - J_CKe)/W_c; 

%f(iPI_c)  = (-Rm_cyto*J_PI2 + J_AtC)/W_c; 
f(iPI_c)  = (-Rm_cyto*J_PI1 + J_AtC)/W_c; 

f(iPCr_c) = (-J_CKe)/W_c;
f(iAMP_c) = 0; %(-Rm_cyto*J_AMP + J_AKe)/W_c;
f(iCr_c)  = (+J_CKe)/W_c;
