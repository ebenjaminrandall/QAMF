function [dxdt,f] = Cell_dXdT(~,x,pars,conc,fixedpars,flags)
...
% Flux
J_PiC = X_PiC * (H_c * HPi_c * (V_c * W_c)^2 - H_x * HPi_x * (V_m * W_x)^2 ) / ... 
    (k_PiC * (1 + (HPi_c * V_c * W_c)/k_PiC) * (1 + (HPi_x * V_m * W_x)/k_PiC));
...
% Membrane potential 
dDPsi = (n_C1 * J_C1 + n_C3 * J_C3 + n_C4 * J_C4 - n_F1F0 * J_F1F0 - J_ANT) / Cm;
...
% Buffer species
dATP_c = ( R_m2c * J_ANT - J_AtC + J_CK) / W_c;  
dADP_c = (-R_m2c * J_ANT + J_AtC - J_CK) / W_c; 
