...
figure(1); plot(t,x(:,1)); title('\Delta\Psi (mV)');
figure(2); plot(t,x(:,iPCr_c)./x(:,iATP_c)); title('CrP/ATP');
figure(3); plot(t,x(:,iPI_c)*1e3,t,x(:,iPI_x)*1e3); title('[Pi]'); legend('cyto','matrix');
figure(4); plot(t,x(:,iNADH_x)./NADtot); title('NADH');
